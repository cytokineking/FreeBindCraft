...
def _get_openmm_forcefield():
    ...
    if _OPENMM_FORCEFIELD_SINGLETON is None:
        _OPENMM_FORCEFIELD_SINGLETON = app.ForceField(
...
def _create_lj_repulsive_force(
    ...
    if lj_rep_base_k_kj_mol > 0 and original_sigmas and lj_rep_ramp_factors:
        ...
        k_rep_lj_param_index = lj_rep_custom_force.addGlobalParameter(
            "k_rep_lj", float(initial_k_rep_val)
        )
        ...
        if nonbonded_force_index != -1:
            ...
            if nb_method in [
                ...
                lj_rep_custom_force.setCutoffDistance(
                    existing_nb_force.getCutoffDistance()
                )
                if nb_method == openmm.NonbondedForce.CutoffPeriodic:
                    lj_rep_custom_force.setUseSwitchingFunction(
                        existing_nb_force.getUseSwitchingFunction()
                    )
                    if existing_nb_force.getUseSwitchingFunction():
                        lj_rep_custom_force.setSwitchingDistance(
                            existing_nb_force.getSwitchingDistance()
                        )
            ...
            for ex_idx in range(existing_nb_force.getNumExceptions()):
                p1, p2, chargeProd, sigmaEx, epsilonEx = (
                    existing_nb_force.getExceptionParameters(ex_idx)
                )
        ...
        for atom in fixer.topology.atoms():
            if atom.name in BACKBONE_ATOM_NAMES:
                ...
                restraint_force.addParticle(
                    atom.index, [xyz_vec[0], xyz_vec[1], xyz_vec[2]]
                )
                num_bb_restrained += 1
...
def _calculate_shape_complementarity(
    pdb_file_path, binder_chain="B", target_chain="A", distance=4.0
):
    """
    ...
    try:
        ...
        vprint(
            f"[SC-RS] Initiating shape complementarity for {basename} (target={target_chain}, binder={binder_chain})"
        )
        ...
        if sc_bin is None:
            ...
            vprint(
                f"[SC-RS] Binary not found; using placeholder value 0.70 for {basename}"
            )
            return 0.70
        else:
        ...
        if isinstance(payload, dict):
            try:
                sc_key = (
                    "sc"
                    if "sc" in payload
                    else ("sc_value" if "sc_value" in payload else None)
                )
                if sc_key is not None:
                    ...
                    if 0.0 <= sc_val <= 1.0:
                        ...
                        vprint(
                            f"[SC-RS] Completed for {basename}: SC={sc_val:.2f} in {elapsed:.2f}s"
                        )
                        return sc_val
            except Exception:
                pass
...
def _compute_sasa_metrics(pdb_file_path, binder_chain="B", target_chain="A"):
    ...
    try:
        ...
        basename = os.path.basename(pdb_file_path)
        vprint(
            f"[SASA-Biopython] Start for {basename} (binder={binder_chain}, target={target_chain})"
        )
        ...
        if binder_chain in complex_model:
            ...
            sr_mono = ShrakeRupley(
                probe_radius=1.40, n_points=960, radii_dict=R_CHOTHIA
            )
            ...
            for residue in binder_only_chain:
                if Polypeptide.is_aa(residue, standard=True):
                    ...
                    if aa1 in HYDROPHOBIC_AA_SET:
                        res_sasa = sum(
                            getattr(atom, "sasa", 0.0) for atom in residue.get_atoms()
                        )
                        hydrophobic_res_sasa += res_sasa
            surface_hydrophobicity_fraction = (
                (hydrophobic_res_sasa / binder_sasa_monomer)
...
def _compute_sasa_metrics_with_freesasa(
    pdb_file_path, binder_chain="B", target_chain="A"
):
    """
    ...
    try:
        ...
        basename = os.path.basename(pdb_file_path)
        vprint(
            f"[SASA-FreeSASA] Start for {basename} (binder={binder_chain}, target={target_chain})"
        )
        ...
        if classifier_obj is not None:
            structure_complex = freesasa.Structure(
                pdb_file_path, classifier=classifier_obj
        ...
        try:
            ...
            sel_area = freesasa.selectArea(
                selection_defs, structure_complex, result_complex
            )  # type: ignore[name-defined]
        ...
        parser = PDBParser(QUIET=True)
        complex_structure_bp = parser.get_structure(
            "complex_for_freesasa", pdb_file_path
        )
        ...
        try:
            if binder_chain in complex_model_bp:
                ...
                if classifier_obj is not None:
                    structure_binder_only = freesasa.Structure(
                        tmp_binder_path, classifier=classifier_obj
                    )  # type: ignore[name-defined]
                else:
                ...
                try:
                    ...
                    with _suppress_freesasa_warnings():
                        sel_area = freesasa.selectArea(
                            sel_defs, structure_binder_only, result_binder_only
                        )  # type: ignore[name-defined]
                    hydro_area = float(sel_area.get("hydro", 0.0))
                    if binder_sasa_monomer > 0.0:
                        surface_hydrophobicity_fraction = (
                            hydro_area / binder_sasa_monomer
                        )
                except Exception:
            ...
            if target_chain in complex_model_bp:
                ...
                if classifier_obj is not None:
                    structure_target_only = freesasa.Structure(
                        tmp_target_path, classifier=classifier_obj
                    )  # type: ignore[name-defined]
                else:
        ...
        return _compute_sasa_metrics(
            pdb_file_path, binder_chain=binder_chain, target_chain=target_chain
        )
...
def openmm_relax(
                ...
                for residue in chain:
                    ...
                    if Polypeptide.is_aa(residue, standard=True):
                        ...
                        if b_factor is not None:
                            ...
                            original_residue_b_factors[(chain.id, residue.id)] = (
                                b_factor
                            )
    ...
    try:
        ...
        for i_force_idx in range(
            system.getNumForces()
        ):  # Use getNumForces and getForce
            ...
            if isinstance(force_item, openmm.NonbondedForce):
                ...
                for p_idx in range(force_item.getNumParticles()):
                    ...
                    original_sigmas.append(
                        sigma.value_in_unit(unit.nanometer)
                    )  # Store as float in nm
                break
            ...
            for attempt_idx in range(1, 4):
                ...
                try:
                    ...
                    if attempt_idx < 3:
                        ...
                        vprint(
                            f"[OpenMM-Relax] Platform {p_name_to_try} failed after {attempt_idx} attempts"
                        )
                        break
        ...
        if restraint_k_kcal_mol_A2 > 0 or lj_rep_base_k_kj_mol > 0:
            ...
            if (
                lj_rep_custom_force is not None
                and k_rep_lj_param_index != -1
                and lj_rep_base_k_kj_mol > 0
            ):
                lj_rep_custom_force.setGlobalParameterDefaultValue(
                    k_rep_lj_param_index, 0.0
                )  # Pass plain float
            ...
            if (
                restraint_force is not None
                and k_restraint_param_index != -1
                and restraint_k_kcal_mol_A2 > 0
            ):
                ...
                restraint_force.setGlobalParameterDefaultValue(
                    k_restraint_param_index, full_initial_restraint_k_val
                )
                restraint_force.updateParametersInContext(simulation.context)

            initial_min_tolerance = (
                openmm_ramp_force_tolerance_kj_mol_nm
                * unit.kilojoule_per_mole
                / unit.nanometer
            )
            simulation.minimizeEnergy(
                tolerance=initial_min_tolerance, maxIterations=openmm_max_iterations
            )
        ...
        effective_restraint_factors = (
            restraint_ramp_factors
            if restraint_k_kcal_mol_A2 > 0 and restraint_ramp_factors
            else [0.0]
        )  # Use 0.0 if no restraint
        effective_lj_rep_factors = (
            lj_rep_ramp_factors
            if lj_rep_base_k_kj_mol > 0 and lj_rep_ramp_factors
            else [0.0]
        ...
        ramp_pairs = list(
            zip_longest(
                effective_restraint_factors, effective_lj_rep_factors, fillvalue=0.0
            )
        )
        ...
        for i_stage_val, (k_factor_restraint, current_lj_rep_k_factor) in enumerate(
            ramp_pairs
        ):
            ...
            if (
                ...
                restraint_force.setGlobalParameterDefaultValue(
                    k_restraint_param_index, numeric_k_for_stage
                )
                restraint_force.updateParametersInContext(simulation.context)
            ...
            force_tolerance_quantity = (
                current_force_tolerance * unit.kilojoule_per_mole / unit.nanometer
            )
            ...
            per_call_max_iterations = (
                200
                if (openmm_max_iterations == 0 or openmm_max_iterations > 200)
                else openmm_max_iterations
            )
            remaining_iterations = openmm_max_iterations
            small_improvement_streak = 0
            last_energy = simulation.context.getState(
                getEnergy=True
            ).getPotentialEnergy()
            ...
            while True:
                ...
                current_energy = simulation.context.getState(
                    getEnergy=True
                ).getPotentialEnergy()
            ...
            if stage_final_energy < best_energy:
                ...
                best_positions = simulation.context.getState(
                    getPositions=True
                ).getPositions(asNumpy=True)  # Use asNumpy=True
        ...
        if original_residue_b_factors:
            try:
                ...
                relaxed_structure_for_bfactors = bio_parser.get_structure(
                    "relaxed_aligned", output_pdb_path
                )
                modified_b_factors = False
                for model in relaxed_structure_for_bfactors:
                    for chain in model:
                        for residue in chain:
                            b_factor_to_apply = original_residue_b_factors.get(
                                (chain.id, residue.id)
                            )
        ...
        vprint(
            f"[OpenMM-Relax] Completed relax for {basename} in {elapsed_total:.2f}s (platform={platform_name_used})"
        )
        ...
        print(
            f"[OpenMM-Relax] ERROR; copied input to output for {basename} after {elapsed_total:.2f}s"
        )
        return platform_name_used
...
def pr_alternative_score_interface(pdb_file, binder_chain="B", sasa_engine="auto"):
    ...
    interface_residues_pdb_ids = [
        f"{binder_chain}{pdb_res_num}" for pdb_res_num in interface_residues_set.keys()
    ]
    interface_residues_pdb_ids_str = ",".join(interface_residues_pdb_ids)
    vprint(
        f"[Alt-Score] Found {len(interface_residues_pdb_ids)} interface residues in {time.time() - t0_if:.2f}s"
    )
    ...
    if str(sasa_engine).lower() == "biopython":
        ...
        (
            ...
            target_sasa_in_complex,
            target_sasa_monomer,
        ) = _compute_sasa_metrics(pdb_file, binder_chain=binder_chain, target_chain="A")
        ...
        (
            ...
            target_sasa_in_complex,
            target_sasa_monomer,
        ) = _compute_sasa_metrics_with_freesasa(
    ...
    try:
        ...
        print(
            f"[Biopython-SASA] WARN interface_target_dSASA for {pdb_file}: {e_idsasa}"
        )
    ...
    interface_binder_fraction = (
        (interface_total_dSASA / binder_sasa_in_complex * 100.0)
    ...
    interface_sc = _calculate_shape_complementarity(
        pdb_file, binder_chain, target_chain="A"
    )
    ...
    vprint(
        f"[Alt-Score] Completed scoring for {basename} in {time.time() - t0_all:.2f}s"
    )
    return interface_scores, interface_AA, interface_residues_pdb_ids_str


def openmm_relax_subprocess(
    pdb_file_path, output_pdb_path, use_gpu_relax=True, timeout=None, max_attempts=3
):
    ...
    code_parts.append(
        "logging.getLogger('pyrosetta.distributed').setLevel(logging.WARNING)"
    )
    code_parts.append(
        "logging.getLogger('pyrosetta.distributed.utility.pickle').setLevel(logging.WARNING)"
    )
    ...
    for attempt_idx in range(1, attempts + 1):
        ...
        if fallback_signature in combined_out and attempt_idx < attempts:
            print(
                f"[OpenMM-Relax] Detected fallback copy; retrying ({attempt_idx + 1}/{attempts})"
            )
            time.sleep(0.5)
            continue
